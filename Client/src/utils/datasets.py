...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        return path, img, img0, [self.w,self.h]
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640):
        ...
        for i, s in enumerate(sources):
            try:
                ...
                cap = cv2.VideoCapture(int(s) if s in ['0','1','2'] else s)
                assert cap.isOpened(), 'Failed to open %s' % s
                self.w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
                self.h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
                self.fps = cap.get(cv2.CAP_PROP_FPS) % 100
                #self.frame_sleep = 1/self.fps if self.fps>0 else 0.04
    ...
    def update(self, index, cap):
        ...
        try:
            while cap.isOpened():
                ...
                time.sleep(self.frame_sleep)  # wait time
                if Face_Detection.pause_vid:
                    return
        except:
            pass
    ...
    def __next__(self):
        try:
            ...
            return self.sources, img, img0, [self.w,self.h,self.fps]
        except:
            pass
